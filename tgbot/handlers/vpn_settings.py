...
import uuid  # можно удалить, если больше не нужен
...
async def _create_or_get_key(message: Message, days: int):
    ...
    # Убедимся, что в локальной БД есть запись
    await create_user(user_id)
    record = await get_user(user_id)

    # 1) Есть действующая подписка и не запрашивают продление
    if record and record.get("sub_id") and record.get("subscription_end"):
        ...
        # если есть «живой» ключ и days==0, просто шлём его
        ...
        # если продление платное и подписка ещё жива
    ...
    # 2) Активный пробный ключ (days==0)
    ...
    # 3) Нет активных ключей и не просят продление
    if days <= 0:
        return await message.answer(
    ...
    # 4) Фолбек: создаём пользователя в Marzban с sub_id = Telegram-ID
    sub_id = str(user_id)
    expire_dt = now + timedelta(days=days)

    # Попробуем получить уже существующую ссылку
    raw_link = None
    try:
    ...
    # Если ссылки нет — создаём пользователя на стороне Marzban
    if not raw_link:
    ...
    # Сохраняем sub_id и ссылку в локальной БД
