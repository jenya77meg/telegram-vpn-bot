...
async def cb_free_trial(callback: CallbackQuery, state: FSMContext):
    ...
    # Гарантируем запись в локальной БД
    await create_user(user_id)
    now = datetime.now(timezone.utc)

    # 3) Создание пробного ключа
    sub_id = str(user_id)  # используем Telegram-ID вместо UUID
    expire_dt = now + timedelta(days=7)

    # Попытка получить уже существующий пробный ключ
    try:
    ...
    # Сохраняем или обновляем пробный ключ в локальной БД
    ...
    return await callback.message.answer(
        ...
        f"🔑 Ваш пробный VLESS-ключ до {expire_dt.strftime('%d.%m.%Y')}:\n"
